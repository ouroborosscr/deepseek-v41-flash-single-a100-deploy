...
本方案是**不做额外低比特二次量化的部署方案**：没有再做 2-bit/3-bit/4-bit
压缩，没有裁剪专家，也没有将 Engram 置零。官方发布的混合精度权重按原有表示保留：
路由专家为 MXFP4、Engram 为 FP8 原始字节、attention/dense 为 Q8_0、embedding
为 BF16、norm 为 F32。因此这里所说的“无量化”是指不额外做低比特量化，并不表示每个张量都是 FP16/BF16。

为了在单卡文本运行时控制规模，转换后的 GGUF **主动去除了视觉编码器/aligner 和
DSpark speculative-draft 草稿头**。因此当前服务是高保真的文本主模型，不是完整的“多模态 + DSpark”发行版。
...
- **没有 DSpark 草稿头：** 跳过了 2,398 个 MTP/DSpark 张量，但普通自回归文本生成仍可用。
- **实验性运行时：** 不是 upstream llama.cpp 或标准 vLLM 路径。
- **上下文上限取决于工作负载和分配器：** 公网服务仍保持保守的 8,192 tokens。
  独立实测中，`786,432` tokens 已完成短请求端到端生成；`849,920` 虽能完成初始化，
  但第一次生成就 CUDA OOM。`1,048,576` 配置在 compute buffer 预留阶段失败，需要
  额外约 9.77 GiB 显存，而 80GB 卡无法提供。模型声明支持 1M，但本单卡方案应表述为
  **实测可服务约 786K**，不能当作 1M 生产保证。
